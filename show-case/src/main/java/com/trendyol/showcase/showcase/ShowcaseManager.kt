...
data class ShowcaseManager private constructor(
    ...
    private val showCasesList:ArrayList<ShowcaseModel>,
    ...
    fun show(activity: Activity, requestCode: Int? = null) {
        ...
        val model = if (resId != null) readFromStyle(activity, resId) else showCasesList
        intent.putParcelableArrayListExtra(ShowcaseActivity.BUNDLE_KEY, showCasesList)
    ...
    fun show(activity: Activity, requestCode: Int? = null, showCasesList:ArrayList<ShowcaseModel>) {
        val intent = Intent(activity, ShowcaseActivity::class.java)
        val model = if (resId != null) readFromStyle(activity, resId) else showCasesList
        intent.putParcelableArrayListExtra(ShowcaseActivity.BUNDLE_KEY, showCasesList)
    ...
    fun show(fragment: Fragment, requestCode: Int? = null) {
        fragment.activity?.let { activity ->
            ...
            intent.putExtra(ShowcaseActivity.BUNDLE_KEY, showCasesList)
    ...
    class Builder {
        ...
        fun addSequenceView(showcaseModel: ShowcaseModel)=apply {
...
////            focusItemView?.getGlobalVisibleRectobalVisibleRect(rect)
//            var targetView=focusView?:focusItemView
...
//
////            if (addToSequence){
////                addSequenceView(showcaseModel)
////            }
//            Log.d("dshowCaseList",sequenceViewList.toString())

            return ShowcaseManager(sequenceViewList[0], sequenceViewList,resId)
        }
